...
// Function to load emergency contacts from a file
void loadEmergencyContactsFromFile(EmergencyContact **head, const char *filename);

// Function to save emergency contacts to a file
void saveEmergencyContactsToFile(EmergencyContact *head, const char *filename);

// Function to delete an emergency contact by name
void deleteEmergencyContact(EmergencyContact **head, const char *name);

// Function to display all emergency contacts
void displayEmergencyContacts(EmergencyContact *head);

// Function to add a new emergency contact
void addEmergencyContact(EmergencyContact **head);
...
int main()
{
    ...
    FILE *file_ptr = NULL;
    loadEmergencyContactsFromFile(&emergencyContacts, "emergency_contacts.txt");
    while (1)
    {
        clearScreen();
...
void showbloodbank()
{
    ...
    printf("\t\t\t\t\tTotal A+ blood: %d\n", cap);
    printf("\t\t\t\t\tTotal A- blood: %d\n", can);
    printf("\t\t\t\t\tTotal B+ blood: %d\n", cbp);
    printf("\t\t\t\t\tTotal B- blood: %d\n", cbn);
    printf("\t\t\t\t\tTotal AB+ blood: %d\n", cabp);
    printf("\t\t\t\t\tTotal AB- blood: %d\n", cabn);
    printf("\t\t\t\t\tTotal O+ blood: %d\n", cop);
    printf("\t\t\t\t\tTotal O- blood: %d\n", con);
    pressEnterToContinue();
    clearScreen();
}
...
void addEmergencyContact(EmergencyContact **head)
{
    ...
    printf("\t\t\t\t\tEnter number: ");
    fgets(newContact->number, MAX_NUMBER_LENGTH, stdin);
    newContact->number[strcspn(newContact->number, "\n")] = '\0'; // Remove trailing newline

    newContact->next = *head;
    *head = newContact;
    printf("Emergency Contacts Successfully Added\n");
    pressEnterToContinue();
}
...
void deleteEmergencyContact(EmergencyContact **head, const char *name)
{
    if (*head == NULL)
    {
        printf("Emergency contacts list is empty.\n");
        return;
    }

    EmergencyContact *current = *head;
    EmergencyContact *prev = NULL;
    ...
    if (current != NULL)
    {
        if (prev == NULL) // If contact to delete is the first node
        {
            *head = current->next;
        }
        else
...
void EmergencyContacts()
{
    ...
    do
    {
        ...
        case 2:
            ...
            break;
        case 3:
            saveEmergencyContactsToFile(emergencyContacts, "emergency_contacts.txt");
        ...
        case 4:
            ...
            deleteEmergencyContact(&emergencyContacts, nameToDelete);
            pressEnterToContinue();
            clearScreen();
            break;
        case 0:
